...
const Dates: React.FC<Props> = ({ timePoints }) => {
  ...
  const rotateCircle = (index: number) => {
    ...
    const delta = gsap.utils.wrap(-180, 180, targetRotation - rotationRef.current);
    ...
    gsap.to(startYearValue.current, {
      ...
      onUpdate: function () {
        if (startYearRef.current) {
          startYearRef.current.textContent = Math.round(startYearValue.current.value).toString();
        }
      },
    });
    ...
    gsap.to(endYearValue.current, {
      ...
      onUpdate: function () {
        if (endYearRef.current) {
          endYearRef.current.textContent = Math.round(endYearValue.current.value).toString();
        }
      },
    });
    ...
    gsap.to(circleRef.current, {
      ...
      ease: "power2.inOut",
      onUpdate: () => {
        rotationRef.current = gsap.getProperty(circleRef.current, "rotation") as number;
        ...
        <div className={styles.circle} ref={circleRef}>
          {timePoints.map((point: TimePoint, index: number) => (
            <div
              ...
              style={{
                transform: `rotate(${index * POINT_ANGLE_STEP}deg) translate(${CIRCLE_DIAMETER / 2}px)`,
              }}
            ...
            >
              ...
              >
                <div className={styles.pointContentText}>{point.id}</div>
                <div className={styles.pointTitle}>{point.subject}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className={styles.circleControls}>
          ...
          <div className={styles.buttons}>
            <ArrowButton
              direction="prev"
              onClick={() => {
                const newIndex = (activeIndex - 1 + timePoints.length) % timePoints.length;
